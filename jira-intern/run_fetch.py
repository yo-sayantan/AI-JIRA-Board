...
from datafile import write_outputs  # noqa: E402
...
REQUIRED_APPROVALS = int((load_config(INTERN).get("app") or {}).get("requiredApprovals") or 2)
...
def pr_to_obj(pr, bb_proj=None):
    ...
    # State per the contract: merged | declined | changes | approved(required threshold, no open comments) | comments.
    if merged:
        pstate = "merged"
        open_c, cr = 0, ct
    elif declined:
