...
export function Footer() {
  return (
    <footer className="fixed inset-x-0 bottom-0 z-30 flex h-9 items-center gap-3 border-t border-[var(--line)] bg-[var(--bg)]/85 px-4 text-[11px] text-[var(--muted)] backdrop-blur-xl md:px-6 lg:px-8">
      ...
      {/* Only when the config names someone — an unset badge would otherwise render as an
          empty pill linking nowhere for anyone who cloned this without setting branding. */}
      {BRANDING.badgeText && (
        <a
          href={BRANDING.badgeUrl || undefined}
          target="_blank"
